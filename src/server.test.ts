...
describe("repo routes", () => {
  it("POST /repos registers a repo from a valid URL", async () => {
    const res = await request(app).post("/repos").send({ url: "https://github.com/owner1/repo1" });
  ...
  it("POST /repos rejects a missing url with 400, not a stack trace", async () => {
    const res = await request(app).post("/repos").send({});
  ...
  it("POST /repos rejects a malformed url with 400", async () => {
    const res = await request(app).post("/repos").send({ url: "not a url" });
    expect(res.status).toBe(400);
  });

  it("POST /repos surfaces upstream fetch failures without crashing", async () => {
    const res = await request(app).post("/repos").send({ url: "https://github.com/owner2/missing-repo" });
  ...
  it("POST /repos/bulk reports per-url success and failure independently", async () => {
    const res = await request(app)
      .post("/repos/bulk")
      .send({ urls: ["https://github.com/owner3/repo3", "https://github.com/owner3/missing-repo", "not a url"] });
  ...
  it("POST /repos/bulk rejects a non-array urls field with 400", async () => {
    const res = await request(app).post("/repos/bulk").send({ urls: "not-an-array" });
    expect(res.status).toBe(400);
  });

  it("GET /repos lists previously registered repos", async () => {
    await request(app).post("/repos").send({ url: "https://github.com/owner4/repo4" });
  ...
  it("DELETE /repos/:id removes a repo, then 404s on a second delete", async () => {
    const created = await request(app).post("/repos").send({ url: "https://github.com/owner5/repo5" });
    const id = created.body.data.id;
    const first = await request(app).delete(`/repos/${id}`);
    expect(first.status).toBe(204);
    const second = await request(app).delete(`/repos/${id}`);
    expect(second.status).toBe(404);
  });

  it("POST /repos/:id/refresh 404s for an unknown id", async () => {
    const res = await request(app).post("/repos/does-not-exist/refresh");
...
describe("graph routes", () => {
  it("GET /graph reflects registered repos", async () => {
    await request(app).post("/repos").send({ url: "https://github.com/owner6/repo6" });
  ...
  it("GET /graph/dependency/:name/history returns a placeholder shape", async () => {
    const res = await request(app).get("/graph/dependency/chalk/history");
    expect(res.status).toBe(200);
...
describe("verification and approval routes", () => {
  it("POST /impact/:dependency/verify 404s when nothing depends on it", async () => {
    const res = await request(app).post("/impact/totally-unused-package/verify").send({ targetVersion: "1.0.0" });
    expect(res.status).toBe(404);
  });

  it("POST /impact/:dependency/verify requires a targetVersion", async () => {
    await request(app).post("/repos").send({ url: "https://github.com/owner7/repo7" });
    const res = await request(app).post("/impact/chalk/verify").send({});
    expect(res.status).toBe(400);
  });

  it("POST /impact/:dependency/verify starts a session and returns its id", async () => {
    await request(app).post("/repos").send({ url: "https://github.com/owner8/repo8" });
    const res = await request(app).post("/impact/chalk/verify").send({ targetVersion: "^5.0.0" });
  ...
  it("GET /sessions/:sessionId/events returns the buffered events for that session", async () => {
    const res = await request(app).get("/sessions/session-1/events");
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body.data)).toBe(true);
  });

  it("POST /sessions/:sessionId/approval requires threadId, toolCallId, and a valid status", async () => {
    const missingFields = await request(app).post("/sessions/session-1/approval").send({});
    expect(missingFields.status).toBe(400);

    const badStatus = await request(app)
      .post("/sessions/session-1/approval")
      .send({ threadId: "main", toolCallId: "call_1", approval: { status: "maybe" } });
    expect(badStatus.status).toBe(400);
  });

  it("POST /sessions/:sessionId/approval accepts an allow decision", async () => {
    const res = await request(app)
      .post("/sessions/session-1/approval")
      .send({ threadId: "main", toolCallId: "call_1", approval: { status: "allow" } });
    expect(res.status).toBe(202);
  });
  ...
  it("POST /sessions/:sessionId/approval accepts a deny decision with a reason", async () => {
    const res = await request(app)
      .post("/sessions/session-1/approval")
      .send({ threadId: "main", toolCallId: "call_1", approval: { status: "deny", reason: "not safe" } });
    expect(res.status).toBe(202);
  });
});
